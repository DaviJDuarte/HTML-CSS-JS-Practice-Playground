...
function convert() {
  ...
  if (document.getElementById('unit').value == 'celsius') {
    ...
    document.getElementById('result').innerHTML = `Temperature in Fahrenheit: ${Math.round(result*10)/10}&degF`;
  }
  else if (document.getElementById('unit').value == 'fahrenheit') {
    var result = (temperature - 32) * 5 / 9;
    document.getElementById('result').innerHTML = `Temperature in Celsius: ${Math.round(result*10)/10}&degC`;
  }
}
